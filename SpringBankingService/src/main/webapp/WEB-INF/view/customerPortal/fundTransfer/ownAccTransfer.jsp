	...
	<div class="container my-4">
		<div class="card card-shadow">
			<div class="card-body p-4">
				<c:choose>
					<c:when test="${custAccList.size() > 1}">
						<div class="bs-stepper py-3 w-50 mx-auto">
							...
							<div class="bs-stepper-content p-0">
								<!-- Contents -->
								<form name="ownTransfer" class="needs-validation" action="own/save" method="post" novalidate>
									...
									<div id="confirmation" class="content" role="tabpanel" aria-labelledby="confirmationTrigger">
										<div class="row mb-3">
											<label for="accFrom" class="col-5 col-lg-3 col-form-label">From</label>
											<div id="accFrom" class="col-12 col-lg-9 col-form-label"></div>
										</div>
										<div class="row mb-3">
											<label for="accTo" class="col-5 col-lg-3 col-form-label">To</label>
					...
					<c:otherwise>
						<div class="text-center">
							<div class="alert alert-danger text-center w-50 m-auto" role="alert">
								<p>
									You only have <strong>1</strong> account.
								</p>
								<p>You cannot perform Own Account Transfer.</p>
							</div>
...
<script>
	...
	let form = document.forms['ownTransfer'];
	...
	// Remove same acc on the accTo selection to avoid transferring to the same acc
	var deletedAccOpt;
	...
	function accFromChange() {
		...
		let accToSelect = form['accTo'];
		for (var i = 0; i < accToSelect.options.length; i++) {
			if (accToSelect.options[i].value == selectedAccFrom.value) {
				if (deletedAccOpt != null)
					accToSelect.add(deletedAccOpt);
				deletedAccOpt = accToSelect.options[i];
				accToSelect.removeChild(accToSelect.options[i]);
			}
		}
	}
