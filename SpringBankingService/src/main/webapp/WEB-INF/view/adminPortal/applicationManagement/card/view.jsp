...
<div class="container w-75 p-2">
	<div class="row justify-content-between">
		<div class="col-auto">
			<h3 class="mt-3">View Card Application</h3>
		</div>
		<div class="col-auto">
			...
			<div class="row justify-content-end align-items-center mb-2">
				<div class="col-auto">
					Status: <span class="badge text-bg-${status} text-white text-capitalize"> <c:out value="${cardApp.status}" />
					</span>
				</div>
				<c:if test="${not empty cardApp.comments}">
					<div class="col-auto">
						<button id="commentsPopover" class="badge border-0 text-bg-secondary" data-bs-toggle="popover"
							data-bs-content="${cardApp.comments}">Click to view comments</button>
					</div>
				</c:if>
			</div>
			<c:if test="${fn:toLowerCase(cardApp.status) != 'approved' and fn:toLowerCase(cardApp.status) != 'rejected'}">
		...
		<div class="bs-stepper-header m-auto mb-3" role="tablist">
			...
			<div class="step" data-target="#creditCardDetails">
				<button type="button" class="step-trigger" role="tab" aria-controls="address" id="creditCardDetailsTrigger">
					<span class="bs-stepper-circle">4</span><span class="bs-stepper-label">Credit Card Details</span>
				</button>
			</div>
			<div class="line"></div>
			...
			<form:form name="applyCard" modelAttribute="cardApp" action="" method="post" enctype="multipart/form-data">
				<div class="card card-shadow">
					<div class="card-body p-4">
						<!-- Contents -->
						<div id="selectedCard" class="content" role="tabpanel" aria-labelledby="selectedCardTrigger">
							<h4 class="mb-4">Selected Card</h4>
							<div class="row g-4 justify-content-center">
								<div class="col-6 col-xl-4">
									<div class="card card-hover h-100">
										<div class="card-thumbnail-img p-1">
											<img id="thumbnailPrev" src="data:image/png;base64,${cardApp.creditCard.getBase64Thumbnail()}"
												class="card-img-top img-fluid" style="object-fit: contain;" alt="${cardApp.creditCard.title}">
										</div>
										<div class="card-body pb-0">
											<h5 id="titlePrev" class="card-title">
												<c:out value="${cardApp.creditCard.title}" />
											</h5>
											<p id="descriptionPrev" class="card-text text-justify">
												<c:out value="${cardApp.creditCard.description}" />
											</p>
											<table class="table text-justify">
												<tr>
													<td>Min Income</td>
													<td><span id="minIncomePrev"> <fmt:formatNumber value="${cardApp.creditCard.minIncome}"
																type="currency" currencyCode="SGD" />
													</span></td>
												</tr>
												<tr>
													<td>Interest Rate</td>
													<td><span id="interestRatePrev"> <fmt:formatNumber
																value="${cardApp.creditCard.interestRate / 100}" type="percent" minFractionDigits="2" />
													</span> p.a.</td>
												</tr>
												<tr>
													<td>Annual Fee</td>
													<td><span id="annualFeePrev"> <fmt:formatNumber value="${cardApp.creditCard.annualFee}"
																type="currency" currencyCode="SGD" />
													</span></td>
												</tr>
											</table>
							...
							<div class="row g-4">
								...
								<div class="col-12 col-md-6">
									<label class="form-label" for="gender">Gender</label><br> <input class="form-control text-capitalize"
										value="${cardApp.gender.toString() == 'm' ? 'Male' : 'Female'}" readonly />
								</div>
								<div class="col-12 col-md-6">
							...
							<div class="row g-4">
								<div class="col-12 col-lg-6">
									...
									<div class="input-group">
										<form:input class="form-control" type="number" path="income" placeholder="Enter your monthly income" readonly="true" />
										<span class="input-group-text">SGD</span>
									</div>
								</div>
								<div class="col-12 col-lg-6">
									<label class="form-label" for="cardDisplayName">Display Name on Card</label>
								...
								<div class="col-12">
									...
									<c:choose>
										<c:when test="${not empty cardApp.creditLimit}">
											<div class="input-group">
												<form:input class="form-control" type="number" path="creditLimit" readonly="true" />
												<span class="input-group-text">SGD</span>
											</div>
										</c:when>
										<c:otherwise>
							...
							<div class="row g-4">
								<div class="col-12">
									<label class="form-label" for="identityDoc">Identity Card / Passport</label> <a
										class="form-control border-0 p-0 text-primary" href="${cardApp.id}/identity-doc" target="_blank">Download</a>
								</div>
								<div class="col-12">
									<label class="form-label" for="payslipDoc">Payslip</label> <a class="form-control border-0 p-0 text-primary"
										href="${cardApp.id}/payslip-doc" target="_blank">Download</a>
								</div>
								<div class="col-12">
									<div class="row justify-content-end">
										<div class="col-auto">
...
<c:if test="${fn:toLowerCase(cardApp.status) == 'rejected' or fn:toLowerCase(cardApp.status) == 'further action'}">
	<script>
		/* Popover */
